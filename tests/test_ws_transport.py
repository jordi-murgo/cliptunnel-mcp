...
# ---------------------------------------------------------------------------
# T11: EncryptedTransport + WebSocket integration tests
# ---------------------------------------------------------------------------

from cliptunnel_mcp.encrypted_transport import EncryptedTransport
from cliptunnel_mcp.crypto import encrypt, decrypt, parse_key
import base64


def make_encrypted_transport(aes_key: bytes) -> EncryptedTransport:
    """Build an EncryptedTransport wrapping a WebSocketTransport."""
    fake = FakeWsRepeater(tokens=["t"])
    ...
    return EncryptedTransport(inner, aes_key)


class TestEncryptedWebSocket(unittest.TestCase):
    def test_encrypted_write_does_not_store_plaintext(self) -> None:
        key = b"0" * 32
        t = make_encrypted_transport(key)
        t.write("plaintext-secret")
        # The inner transport's fake received encrypted data, not plaintext
        inner = t._inner  # access the wrapped transport
        self.assertNotEqual(inner.read(), "plaintext-secret")
        t.close()

    def test_encrypted_read_decrypts_on_output(self) -> None:
        key = b"0" * 32
        t = make_encrypted_transport(key)
        t.write("roundtrip-test")
        # EncryptedTransport.read() should decrypt back to plaintext
        self.assertEqual(t.read(), "roundtrip-test")
        t.close()

    def test_encrypted_revision_tracks_writes(self) -> None:
        key = b"0" * 32
        t = make_encrypted_transport(key)
        self.assertEqual(t.revision, 0)
        t.write("first")
    ...
    def test_encrypted_wait_for_change(self) -> None:
        key = b"0" * 32
        t = make_encrypted_transport(key)
        rev = t.revision
    ...
    def test_encrypted_backend_name(self) -> None:
        key = b"0" * 32
        t = make_encrypted_transport(key)
        self.assertEqual(t.backend_name, "encrypted:websocket")
        t.close()

    def test_encrypted_satisfies_transport_protocol(self) -> None:
        key = b"0" * 32
        t = make_encrypted_transport(key)
        self.assertIsInstance(t, Transport)
    ...
    def test_factory_returns_encrypted_for_websocket_with_aes(self) -> None:
        """build_transport() with CLIPTUNNEL_TRANSPORT=websocket and AES key
        returns EncryptedTransport wrapping WebSocketTransport."""
        import os
        from cliptunnel_mcp.transport_factory import build_transport

        key_raw = base64.b64encode(b"0" * 32).decode()
        old_transport = os.environ.get("CLIPTUNNEL_TRANSPORT")
        old_url = os.environ.get("CLIPTUNNEL_WS_URL")
        old_token = os.environ.get("CLIPTUNNEL_WS_TOKEN")
        old_aes = os.environ.get("CLIPTUNNEL_AES_KEY")

        os.environ["CLIPTUNNEL_TRANSPORT"] = "websocket"
        os.environ["CLIPTUNNEL_WS_URL"] = "ws://relay.example.com:9000"
        os.environ["CLIPTUNNEL_WS_TOKEN"] = "secret"
        os.environ["CLIPTUNNEL_AES_KEY"] = key_raw
        try:
            t = build_transport()
            ...
            for key, old in [
                ("CLIPTUNNEL_TRANSPORT", old_transport),
                ("CLIPTUNNEL_WS_URL", old_url),
                ("CLIPTUNNEL_WS_TOKEN", old_token),
                ("CLIPTUNNEL_AES_KEY", old_aes),
            ]:
                if old is not None:
                    os.environ[key] = old
                else:
